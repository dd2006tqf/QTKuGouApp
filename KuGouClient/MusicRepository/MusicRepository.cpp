...
void MusicRepository::initButtonGroup()
{
    ...
    std::shuffle(this->m_musicData.begin(),
                 this->m_musicData.end(),
...
void MusicRepository::initUi()
{
                                        ...
                                        [this]() {
                                            ...
                                            QFile file(
                                                GET_CURRENT_DIR + QStringLiteral(
                                                    "/musicrepo.json")); ///< 加载 JSON 文件
                                            if (!file.open(QIODevice::ReadOnly)) {
                                                qWarning() <<
                                                    "Could not open file for reading musicrepo.json";
                                                STREAM_WARN() <<
                                                    "Could not open file for reading musicrepo.json";
                                                ///< 记录警告日志
                                                return data;
    ...
    Async::onResultReady(future,
                         this,
                         [this](const QList<QJsonObject> &datas) {
                             if (datas.isEmpty()) {
                             ...
                             for (int i = 1; i <= std::min(60, static_cast<int>(datas.size())); ++
                                  i) {
                                 this->m_musicData.emplace_back(
                                     QString(
                                         QString(RESOURCE_DIR) +
                                         "/blockcover/music-block-cover%1.jpg").arg(i),
                             ...
                             for (int i = 1; i <= 40; ++i) {
                                 this->m_videoVector.emplace_back(
                                     QString(
                                         QString(RESOURCE_DIR) +
                                         "/rectcover/music-rect-cover%1.jpg").arg(i),
...
void MusicRepository::initSelectWidget()
{
    ...
    unsigned seed = std::chrono::system_clock::now().time_since_epoch().count(); ///< 获取时间种子
    std::shuffle(this->m_videoVector.begin(),
                 this->m_videoVector.end(),
                 std::default_random_engine(seed)); ///< 随机打乱
    // 控件数组，便于循环处理
...
void MusicRepository::resizeEvent(QResizeEvent *event)
{
    ...
    if (newVisibleState != lastVisibleState) {
        ...
        case 0: ui->block_widget6->hide(); ///< 隐藏块 6
            ui->block_widget7->hide();     ///< 隐藏块 7
            ui->video_widget4->hide();     ///< 隐藏视频 4
            ui->video_widget5->hide();     ///< 隐藏视频 5
            ui->video_widget9->hide();     ///< 隐藏视频 9
            ui->video_widget10->hide();    ///< 隐藏视频 10
            break;
        case 1: ui->block_widget6->show(); ///< 显示块 6
            ...
            break;
        case 2: ui->block_widget6->show(); ///< 显示块 6
