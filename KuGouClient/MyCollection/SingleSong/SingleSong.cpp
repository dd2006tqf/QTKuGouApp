...
void SingleSong::initUi()
{
    ...
    ///< 创建下载按钮工具提示
    collect_download_toolButton_toolTip->setToolTip(QStringLiteral("下载")); ///< 设置下载提示
    auto collect_share_toolButton_toolTip = new ElaToolTip(ui->collect_share_toolButton);
    ///< 创建分享按钮工具提示
    collect_share_toolButton_toolTip->setToolTip(QStringLiteral("分享")); ///< 设置分享提示
    auto collect_album_toolButton_toolTip = new ElaToolTip(ui->collect_album_toolButton);
    ///< 创建专辑按钮工具提示
    collect_album_toolButton_toolTip->setToolTip(QStringLiteral("专辑")); ///< 设置专辑提示
    auto collect_sort_toolButton_toolTip = new ElaToolTip(ui->collect_sort_toolButton);
    ///< 创建排序按钮工具提示
    collect_sort_toolButton_toolTip->setToolTip(QStringLiteral("当前排序方式：默认排序")); ///< 设置默认排序提示
    auto collect_batch_toolButton_toolTip = new ElaToolTip(ui->collect_batch_toolButton);
    ///< 创建批量操作按钮工具提示
    collect_batch_toolButton_toolTip->setToolTip(QStringLiteral("批量操作")); ///< 设置批量操作提示
    connect(m_sortOptMenu,
            &SortOptionMenu::defaultSort,
            this,
            [this, collect_sort_toolButton_toolTip](const bool &down) {
                Q_UNUSED(down);
    ...
    connect(m_sortOptMenu,
            ...
            [this, collect_sort_toolButton_toolTip](const bool &down) {
                ...
                collect_sort_toolButton_toolTip->setToolTip(
    ...
    connect(m_sortOptMenu,
            ...
            [this, collect_sort_toolButton_toolTip](const bool &down) {
                ...
                collect_sort_toolButton_toolTip->setToolTip(
                    down ? QStringLiteral("当前排序方式：歌手降序") : QStringLiteral("当前排序方式：歌手升序")); ///< 更新提示
            });
    connect(m_sortOptMenu,
    ...
    connect(m_sortOptMenu,
            ...
            this,
            [this, collect_sort_toolButton_toolTip] {
                onRandomSort(); ///< 调用随机排序
...
bool SingleSong::eventFilter(QObject *watched, QEvent *event)
{
    ...
    if (const auto button = qobject_cast<QToolButton *>(watched);
        button && button->defaultAction() == this->m_searchAction) {
        if (event->type() == QEvent::Enter) {
            this->m_searchAction->setIcon(
