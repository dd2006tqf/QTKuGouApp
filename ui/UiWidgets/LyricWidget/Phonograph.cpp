...
void Phonograph::paintEvent(QPaintEvent* event)
{
    ...
    painter.drawPixmap(-albumCoverWidth / 2,
                       -albumCoverHeight / 2,
                       albumCoverWidth,
                       albumCoverHeight,
                       picAlumCover);
    ...
    painter.drawPixmap(-rotatePointOfArm.x(),
                       -rotatePointOfArm.y(),
                       toneArmWidth,
                       toneArmHeight,
                       picToneArm);
...
void Phonograph::initPaintingData(bool firstTime)
{
    if (firstTime)
    {
        ...
        picDisk = disk.scaled(albumWidth,
                              albumHeight,
                              Qt::IgnoreAspectRatio,
                              Qt::SmoothTransformation);

        picToneArm = toneArm.scaled(toneArmWidth,
                                    toneArmHeight,
                                    Qt::IgnoreAspectRatio,
                                    Qt::SmoothTransformation);
    ...
    picAlumCover = AlbumCover.scaled(albumCoverWidth,
                                     albumCoverHeight,
                                     Qt::IgnoreAspectRatio,
                                     Qt::SmoothTransformation);
}